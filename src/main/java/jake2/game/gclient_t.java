...
public class gclient_t implements GameClient {
	private static final Logger LOGGER = LoggerFactory.getLogger(gclient_t.class);
	...
	public void read(QuakeFile f, edict_t[] edicts, GameExportsImpl gameExports) throws IOException
	{
		...
		if (f.readInt() != 8765)
			LOGGER.error("jake2.game client load failed for num={}", getIndex());
	}
