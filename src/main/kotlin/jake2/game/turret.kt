...
private val turretBreachThink = registerThink("turret_breach_think") { self, game ->
    ...
    if (self.owner != null) {
        ...
        target[0] = Math3D.snapToEights((self.s.origin[0] + cos(angle) * self.owner.move_origin[0]))
        target[1] = Math3D.snapToEights((self.s.origin[1] + sin(angle) * self.owner.move_origin[0]))
        ...
        val targetZ = Math3D.snapToEights((self.s.origin[2] + self.owner.move_origin[0] * tan(angle) + self.owner.move_origin[2]))
